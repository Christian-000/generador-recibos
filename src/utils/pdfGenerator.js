...
ipcMain.on("generate-pdf", (event, { formData, outputPath }) => {
  (async () => {
    const template = templateJson;
    const { day, month, year, name, amount, concepto } = formData;
    let amountInLetter = getAmountInLetter(amount);
    ...
    // Obtén el directorio del escritorio del usuario
    const desktopDir = path.join(os.homedir(), "Desktop");

    // Construye la ruta completa al archivo PDF
    const pdfPath = path.join(desktopDir, `${formattedDate}.pdf`);

    // Guarda el archivo PDF en el escritorio
...
function getAmountInLetter(num) {
  var data = {
    numero: num,
